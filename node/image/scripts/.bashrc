...
# managed-by=devspaces-node-image
[storage]
driver = "vfs"
...
# Keep npm auth/config and caches in persistent storage to survive pause/resume cycles
if [ -d /home/user/persistent ] && [ -w /home/user/persistent ]; then
    export NPM_CONFIG_USERCONFIG="${NPM_CONFIG_USERCONFIG:-/home/user/persistent/.npmrc}"
    export NPM_CONFIG_CACHE="${NPM_CONFIG_CACHE:-/home/user/persistent/.npm}"
else
    export NPM_CONFIG_USERCONFIG="${NPM_CONFIG_USERCONFIG:-/home/user/.npmrc}"
fi

# Node environment (multi-version: 22 / 24 / 25)
if [ -d /home/user/persistent ] && [ -w /home/user/persistent ]; then
...
# Node version switching must run in the current shell to change PATH, so the
# CLI binaries are wrapped by shell functions here (the base .bash_aliases does
# not know about Node).
if ! declare -F nodeuse >/dev/null 2>&1; then
        nodeuse() {
                /usr/local/bin/nodeuse "$@" || return $?
...
if ! declare -F nodelist >/dev/null 2>&1; then
        nodelist() {
                /usr/local/bin/nodelist "$@"
        }
fi
